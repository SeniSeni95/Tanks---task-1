#include "GameManager7.h"  // Changed from GameManager.h
...
GameManager7::GameManager7(PlayerFactory playerFactory,  // Changed from GameManager
    ...
    if (verboseOutput) {
        std::string outName = "output_verbose_GM7.txt";  // GM7 specific filename
        verboseFile.open(outName);
        if (!verboseFile.is_open()) {
            std::cerr << "[ERROR] GM7: could not open verbose log file" << std::endl;
            verboseOutput = false;
        }
    }
    
    if (DEBUG_ENABLED) {
        std::cout << "[DEBUG] GM7: GameManager7 initialized with max turns per tank = " 
                  << MAX_TURNS_PER_TANK << std::endl;
    }
}

GameResult GameManager7::run(  // Changed from GameManager
    ...
    if (DEBUG_ENABLED) {
        std::cout << "[DEBUG] GM7: Building board from SatelliteView...\n";
    }
...
for (size_t i = 0; i < map_width; ++i) {
    ...
    if (algo) {
        ...
        if (DEBUG_ENABLED) {
            std::cerr << "[ERROR] GM7: Tank factory returned nullptr for P"
...
// Initialize turn counters for each tank (GM7 specific)
for (const auto& t_ptr : board->tanks) {
    tank_turn_counters[t_ptr.get()] = 0;
}

if (DEBUG_ENABLED) {
    std::cout << "[DEBUG] GM7: Board created, tanks=" << board->tanks.size() << "\n";
}
...
if (satview) {
    ...
    if (DEBUG_ENABLED) {
        std::cout << "[DEBUG] GM7: SatelliteView initialized with board copy\n";
    }
}

    std::ofstream game_output;
    if (verboseOutput) {
        std::string output_filename = "output_GM7_" + map_name + ".txt";  // GM7 specific filename
        game_output.open(output_filename);
        if (!game_output.is_open()) {
            std::cerr << "[ERROR] GM7: Failed to open verbose log file: " << output_filename << std::endl;
            verboseOutput = false;
        }
    }
    ...
    if (DEBUG_ENABLED) {
        std::cout << "[DEBUG] GM7: Entering game loop (max_steps=" << max_steps 
    ...
    while (time_out_steps >= 0 && !game_over && round_counter < (int)max_steps) {
        if (DEBUG_ENABLED) {
            std::cout << "\n[DEBUG] GM7: --- Round " << (round_counter+1) << " ---\n";
        }
        ...
        if (out_of_shells) {
            time_out_steps--;
            if (DEBUG_ENABLED) {
                std::cout << "[DEBUG] GM7: All tanks are out of shells. time_out_steps=" 
                          << time_out_steps << "\n";
            }
        }
        ...
        for (size_t i = 0; i < tanks_by_birth.size(); ++i) {
            ...
            if (t->alive) {
                // GM7 specific: Check if tank has exceeded max turns
                if (tank_turn_counters[t] >= MAX_TURNS_PER_TANK) {
                    ...
                    if (DEBUG_ENABLED) {
                        std::cout << "[DEBUG] GM7: Tank P" << t->player_number 
                                  << " T" << t->tank_number 
                ...
                turn_success[i] = true;
                tank_turn_counters[t]++; // GM7 specific: Increment turn counter
        ...
        if (DEBUG_ENABLED) {
            ...
            std::cout << "[DEBUG] GM7: End of round " << round_counter 
    ...
    if (DEBUG_ENABLED) {
        std::cout << "[DEBUG] GM7: Game finished after " << result.rounds 
...
std::string GameManager7::commandStringToEnumName(const std::string& cmd) {  // Changed from GameManager
...
std::string GameManager7::actionToString(ActionRequest action) {  // Changed from GameManager
    switch (action) {
...
ActionRequest GameManager7::stringToAction(const std::string& actionStr) {  // Changed from GameManager
