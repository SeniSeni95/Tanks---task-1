#include "GameManager8.h"  // Changed from GameManager.h
...
GameManager8::GameManager8(PlayerFactory playerFactory,  // Changed from GameManager
    ...
    if (verboseOutput) {
        std::string outName = "output_verbose_GM8.txt";  // GM8 specific filename
        verboseFile.open(outName);
        if (!verboseFile.is_open()) {
            std::cerr << "[ERROR] GM8: could not open verbose log file" << std::endl;
            verboseOutput = false;
        }
    }
    
    if (DEBUG_ENABLED) {
        std::cout << "[DEBUG] GM8: GameManager8 initialized with max turns per tank = " 
                  << MAX_TURNS_PER_TANK << std::endl;
    }
}

GameResult GameManager8::run(  // Changed from GameManager
    ...
    if (DEBUG_ENABLED) {
        std::cout << "[DEBUG] GM8: Building board from SatelliteView...\n";
    }
...
for (size_t i = 0; i < map_width; ++i) {
    ...
    if (algo) {
        ...
        if (DEBUG_ENABLED) {
            std::cerr << "[ERROR] GM8: Tank factory returned nullptr for P"
...
// Initialize turn counters for each tank (GM8 specific)
for (const auto& t_ptr : board->tanks) {
    tank_turn_counters[t_ptr.get()] = 0;
}

if (DEBUG_ENABLED) {
    std::cout << "[DEBUG] GM8: Board created, tanks=" << board->tanks.size() << "\n";
}
...
if (satview) {
    ...
    if (DEBUG_ENABLED) {
        std::cout << "[DEBUG] GM8: SatelliteView initialized with board copy\n";
    }
}

    std::ofstream game_output;
    if (verboseOutput) {
        std::string output_filename = "output_GM8_" + map_name + ".txt";  // GM8 specific filename
        game_output.open(output_filename);
        if (!game_output.is_open()) {
            std::cerr << "[ERROR] GM8: Failed to open verbose log file: " << output_filename << std::endl;
            verboseOutput = false;
        }
    }
    ...
    if (DEBUG_ENABLED) {
        std::cout << "[DEBUG] GM8: Entering game loop (max_steps=" << max_steps 
    ...
    while (time_out_steps >= 0 && !game_over && round_counter < (int)max_steps) {
        if (DEBUG_ENABLED) {
            std::cout << "\n[DEBUG] GM8: --- Round " << (round_counter+1) << " ---\n";
        }
        ...
        if (out_of_shells) {
            time_out_steps--;
            if (DEBUG_ENABLED) {
                std::cout << "[DEBUG] GM8: All tanks are out of shells. time_out_steps=" 
                          << time_out_steps << "\n";
            }
        }
        ...
        for (size_t i = 0; i < tanks_by_birth.size(); ++i) {
            ...
            if (t->alive) {
                // GM8 specific: Check if tank has exceeded max turns
                if (tank_turn_counters[t] >= MAX_TURNS_PER_TANK) {
                    ...
                    if (DEBUG_ENABLED) {
                        std::cout << "[DEBUG] GM8: Tank P" << t->player_number 
                                  << " T" << t->tank_number 
                ...
                turn_success[i] = true;
                tank_turn_counters[t]++; // GM8 specific: Increment turn counter
        ...
        if (DEBUG_ENABLED) {
            ...
            std::cout << "[DEBUG] GM8: End of round " << round_counter 
    ...
    if (DEBUG_ENABLED) {
        std::cout << "[DEBUG] GM8: Game finished after " << result.rounds 
...
std::string GameManager8::commandStringToEnumName(const std::string& cmd) {  // Changed from GameManager
...
std::string GameManager8::actionToString(ActionRequest action) {  // Changed from GameManager
    switch (action) {
...
ActionRequest GameManager8::stringToAction(const std::string& actionStr) {  // Changed from GameManager
