...
#include <iostream>
    ...
    void game()
    {
        ...
        if (board->tanks.size() == 1)
        {
            ...
            {
                cout << "Player 1 wins!" << std::endl; // Player 1 wins
                game_output << "Player 1 wins!" << std::endl; // Player 1 wins
            }
            else
            {
                cout << "Player 2 wins!" << std::endl; // Player 2 wins
        ...
        else
        {
            ...
            cout << "It's a tie!" << std::endl; // It's a tie
        }
    ...
    string ask_algorithm(char player)
    {
        ...
        for (tank *t : board->tanks)
        {
            if (t->symbol == player)
            {
                std::pair<std::string, double> result = t->algo->decide_move(board, t, LOOKAHEAD_DEPTH); // Get the move from the algorithm
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j < m; ++j)
        {
            ...
            {
                // Place Tank 1
                tank *tank1 = new tank('1', 0, -1, &board.arr[i][j], algo1);
                board.add_tank(tank1);
            }
            else if (cell == '2')
            {
                // Place Tank 2
                tank *tank2 = new tank('2', 0, 1, &board.arr[i][j], algo2);
